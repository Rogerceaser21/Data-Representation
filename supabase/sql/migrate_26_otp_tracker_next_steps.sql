-- otp-v0.12 preview 2 (P2-A) Teacher Tracker: each history entry gains 'next_steps'
-- (the non-empty next_step_1..3, in order) so the expanded row can show what was
-- agreed instead of the old repeated icon history. Additive only: replaces
-- get_otp_tracker() (migrate_25) with the SAME shape plus one new key per history
-- entry; nothing else altered, dropped or replaced. Apply: the foreman applies
-- this file (builders never touch the database).
begin;
    ...
    select yr.teacher_key as tkey,
      jsonb_agg(
        jsonb_build_object(
          ...
          -- next_steps (new, P2-A): the non-empty next_step_1..3, in order. Built from a
