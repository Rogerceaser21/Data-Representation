...
- **How.** The picker is built client-side from `D.rounds` (no new data path). The state line uses a new light anon counts RPC `get_round_status(round)` (`~/AIS-Data-Dashboard/db/migrate_13_round_status.sql`) so the check does not download the heavy narrative/coaching payloads.
- Scope: admin Settings regeneration panel only. R3 form, runner, queue RPCs and every other board untouched. Rollback: `dash-v0.61`.

## v0.61 . 2026-07-06
- **Round regeneration button (admin only, Route A).** Settings gains a "Round regeneration" panel. When a new inspection round has landed (R3 form to Supabase) and been set, one button regenerates every AI-written surface for that round, the Snapshot narrative and insights, the report references, the recommendations and every teacher's coaching note, then holds the result for review and publishes to the live dashboard only on approval. It runs the certified pipeline (Claude Sonnet 5, quotes checked verbatim, judged for fabrication); nothing that fails the checks can be published.
- **How it works.** The button enqueues a job via the password-gated `admin_enqueue_regen` RPC (`~/AIS-Data-Dashboard/db/migrate_12_regen_jobs.sql`: `regen_jobs` + `admin_get_regen_jobs` / `admin_request_publish` / `admin_cancel_regen`, all reusing the shared `verify_admin_password` gate). A local runner (`~/AIS-Data-Dashboard/db/regen_runner.mjs`) on the Mac claims the job, runs the regen chain + judge to FILES (never live during regen, the locked files-only isolation), marks it "ready", then on Publish seeds the approved files into Supabase round-scoped and sets `current_round`. The panel polls and shows live status, stage, progress, the judge summary, a Cancel and a Publish to live.
- **Route A now; Route B (Mac Max plan via `claude -p`) follows.** Both engines run the same certified Sonnet 5. Single active job at a time; publish is a separate, explicit step behind a confirm.
- Scope: admin Settings only (invisible to normal viewers). Snapshot, Coverage, Observations, portal, reports, data untouched. Rollback: `dash-v0.60`.
...
- **Teacher portal = the round STACK.** One summary card per APPROVED round, newest first; each card opens into ONLY that round's session cards, gauges, spectrum and full note (all round-scoped, teacher view: no verbatim, no record links, no admin buttons). Unapproved rounds are entirely absent; nothing approved shows the quiet awaiting line. This kills the v0.56 leak where the unapproved February session card rode along inside the approved June record.
- Data plumbing: `get_teacher_next_steps` is now fetched for every round (live + baked); notes are grouped by round in the client. Future rounds (September onward) slot in automatically once their notes are approved.
- Scope: Observations Next Steps area, Teacher portal below-hero section, export tooling. Snapshot, Coverage, reports, hero, searches untouched. Rollback: `dash-v0.57`.
...
- **Staff photos pipeline (Phase 1).** Portraits flow from the staff Drive folder into Supabase Storage nightly (Apps Script mirror under admin.user, checksum-skipped, deletions propagate); 124 of 149 matched automatically, the rest hand-mappable via photo_alias. Photos never touch the public Pages repo.
- **Department Level is live in the R3 Obs menu.** Humanities renders as a department board: navy header (Humanities, Daniel Van Wyk . Head of Department, the team's portrait stack, teachers observed, department average word, the six-point spread), then a teacher-by-type matrix with tinted SEAS chips; clicking a teacher opens their portal page. Departments appear automatically as they are marked in the staff Sheet; everything else shows the calm "landing with the new academic year" note.
- Scope: new portal board + the Observations menu; Snapshot, Coverage, reports and Settings unchanged. Rollback: `dash-v0.54`.
...
- Scope: the Observations drill-down (session card + coaching note). Snapshot, Coverage, report unchanged. Front-end only; no notes regenerated. Rollback: `dash-v0.50`.

## v0.50 . 2026-06-28
- **Per-teacher coaching note reworked** (the v0.49 engine, presentation + grounding overhauled; Igor's review). All 104 June notes regenerated and re-checked.
- **Facilitating now uses the same six-point scale as Progress** (Very Weak..Outstanding), driven by Teaching r3_05 and floored at Good, so it reads e.g. "Good to Very Good" instead of the old five band names (Beginner/Emerging/Good/Great/Outstanding). The five-point rubric descriptors stay internal only as the source of the coaching move; the band names never appear in the prose.
- **Rubric reference reworded and linked.** Each move now states its grounding in plain English ("In the AIS OTP Progress Rubric, to facilitate better than expected progress at a Good level, the teacher [descriptor]") with an inline link to the rubric, replacing the cryptic "AIS rubric . Facilitating: ..." line.
- **In-house examples removed; reputable online techniques added.** The "what this looks like elsewhere" examples (which linked to other teachers' R3 records, an anonymity risk) are gone. Each move now carries a classroom technique written out in full plus a link to its source, drawn from 18 verified free resources across six themes (Edutopia, Structural Learning, Chartered College, Third Space, InnerDrive on Rosenshine, Tips for Teachers).
- **New layout: two-column focus cards.** Each move splits the action (the move, what to look for, the technique and its source) on the left from its grounding (the rubric line and a verbatim quote from the teacher's own lessons) on the right, with a current-to-target SEAS ladder per aspect and a colour-coded theme tag per move. Chosen from a three-way preview.
- Tooling: `coach_prep` now attaches the online sources and the six-point facilitating display; `coach_verify` anti-fab also checks every rubric line is verbatim rubric text and every source link is one of the 18. The Google Doc coaching export was updated to match (online technique + source link instead of record links).
- Scope: the Observations drill-down + the per-teacher Doc. Snapshot, Coverage and the governance report are unchanged. Rollback: `dash-v0.49`.

## v0.49 . 2026-06-27
- **Per-teacher coaching ("Next Steps & Improvement").** Every teacher observed in the June R3 round now has a personal coaching note in the Observations drill-down. It is generated from that teacher's own observation evidence (their strengths, areas to develop and observer notes, pooled across all their lessons, best rating winning) and the AIS "Progress in Lessons" rubric, and coaches BOTH rubric aspects: Progress in Lessons (the outcome) and Facilitating better-than-expected progress (the practice). It shows where they are and the next level up on each ladder (floored at Good), a short summary, and two to four concrete moves, each tied to a next-band rubric descriptor and a verbatim line from their own lessons, with one or two anonymous same-subject examples from other lessons this round (each linking to the locked R3 record).
- **AI-drafted, coach-approved.** Notes are generated as drafts by a multi-agent batch over the ~104 teachers, with every quote verified verbatim against the source (anti-fabrication). They are visible to everyone with the dashboard password; editing and approving are admin-only (the same admin password as the Round cog), and each note exports to a personal Google Doc to share with the teacher. Per-role access (so only a teacher and their coach see their note) follows with @ais.ae sign-in.
- Backend: a round-scoped `teacher_next_steps` table + a `coaching_rubric` table (both rubric aspects, all bands), an anon read RPC and an admin-gated edit/approve RPC (`migrate_08`). Notes are also baked into the offline `data.js` fallback (gitignored, never on Pages). The Google Doc builder gained a `coaching` layout.
- Scope: the Observations drill-down + the per-teacher Doc export. Snapshot, Coverage and the governance report are unchanged. Rollback: `dash-v0.48`.
