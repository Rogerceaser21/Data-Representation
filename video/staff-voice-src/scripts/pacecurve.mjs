...
const openSpeech = Math.max(sub[0].speech, sub[1].speech);
const openGross = Math.max(sub[0].gross, sub[1].gross);
const band = openSpeech * 1.12;
...
console.log(`\nopening (first two >=20-word scenes): SPEECH ${openSpeech.toFixed(0)} wpm, gross ${openGross.toFixed(0)} wpm` +
  `  ->  accept band <= ${band.toFixed(0)} SPEECH wpm (+12%)`);
...
console.log(over.length ? `OVER BAND: ${over.map(r => `${r.id} ${r.speech.toFixed(0)}`).join(', ')}` : 'all substantial scenes inside the band');
