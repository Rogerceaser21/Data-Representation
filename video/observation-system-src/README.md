...
A 3m23s narrated explainer of the R3 pipeline and where Steve's two KPIs
...
take ships at its natural recorded pace) and added five gold spotlight moments
where the narration names an on-screen element. v1.5 replaced the music (the
ambient pad read as sleepy), added a silent 8s end card pointing viewers to the
rest of the page, and restructured the companion page (video first, sticky
while playing).

**Music:** `assets/music.mp3` is "Corporate Explainer Video" by alex-morgan
from Pixabay (id 568180, Pixabay Content License: free for commercial use, no
attribution required), picked from three measured register candidates Igor
auditioned (upbeat corporate / minimal tech pulse / cinematic documentary),
trimmed to the composition length, faded, and pre-baked to -31 LUFS (~15 dB
under the -16 LUFS narrator) so what previews is what renders. To swap the
track, replace the file with another bed processed the same way; do not raise
its level without re-checking the narrator stays fully intelligible.
