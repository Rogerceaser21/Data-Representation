/* 7 . the dashboard story . gate wipe, the Snapshot Story board, then present mode.
   Fractions are word share through the v2.3 line (82 words).
...
tl.fromTo('#s7gate', { opacity: 1 },
    { opacity: 1, ease: 'none', immediateRender: false,
      duration: atf('s07_story', 0.220) - at('s07_story', -0.55) }, at('s07_story', -0.55))
  ...
  /* "safety as a priority": the gate's own Confidential kicker lifts to gold and
     back (yoyo, so it rests at the gate's authored red). Literal hexes are the
     gate's own palette, #EF343A and #FFBA14, verbatim from s07_story.css. */
  .fromTo('#sc07 .s7gkick', { color: '#EF343A' },
    { color: '#FFBA14', duration: 0.5, ease: 'power2.inOut', yoyo: true, repeat: 1, immediateRender: false }, atf('s07_story', 0.171))
  /* "As an example": the button takes the press, then the gate lifts off the board */
  .fromTo('#s7gbtn', { scale: 1 }, { scale: 0.955, duration: 0.13, ease: 'power2.in', immediateRender: false }, atf('s07_story', 0.206))
  .to('#s7gbtn', { scale: 1, duration: 0.3, ease: 'back.out(2)' }, atf('s07_story', 0.206) + 0.13)
  .to('#s7gate', { yPercent: -100, opacity: 0, duration: 0.85, ease: 'power3.inOut' }, atf('s07_story', 0.220));
...
/* the board lands BEHIND the gate, a beat before the wipe starts, so the lifting
   ...
   left column is never a hole; the sync map's 0.329 "the story plays itself" beat
   would leave it one for ~3s, and the count-up below carries that phrase instead. */
tl.from('#sc07 .s7top', { opacity: 0, y: 18, duration: 0.75, ease: 'power2.out' }, atf('s07_story', 0.198))
  ...
  .from('#sc07 .s7dcard', { opacity: 0, x: 26, duration: 0.75, ease: 'power2.out' }, atf('s07_story', 0.235));
/* "a hundred and eleven lessons", the second tile still counting on "a hundred and
   five teachers", both settled by "counted up live from the data" (0.500) */
countUp('#s7stats .stat .v[data-count]', atf('s07_story', 0.378), 1.5);
...
tl.from('#s7gauge .arcfill', { opacity: 0, duration: 0.3, stagger: 0.022, ease: 'none' }, atf('s07_story', 0.573))
  .from('#s7gauge .arcw', { opacity: 0, scale: 0.8, transformOrigin: '50% 50%', duration: 0.5, ease: 'back.out(1.8)' }, atf('s07_story', 0.596))
  .from('#sc07 .s7gaugecard .gauge-cap', { opacity: 0, y: 10, duration: 0.5 }, atf('s07_story', 0.603))
  ...
  .from('#s7dots .dsdot', { opacity: 0, scale: 0.35, duration: 0.55, ease: 'back.out(1.35)',
    ...
    stagger: { each: 0.009 } }, atf('s07_story', 0.585))
  /* "What is working." / "Where to focus next." */
  .from('#s7rail .s7beat:nth-child(1)', { opacity: 0, x: -14, duration: 0.5 }, atf('s07_story', 0.671))
  .from('#s7rail .s7beat:nth-child(2)', { opacity: 0, x: -14, duration: 0.5 }, atf('s07_story', 0.707))
  .from('#s7rail .subtle', { opacity: 0, duration: 0.6 }, atf('s07_story', 0.735));
...
tl.to('#s7tabs .s7tab', { opacity: 0.55, duration: 0.45 }, atf('s07_story', 0.756))
  ...
  /* "in full screen", holding through "showcase the round by itself in any
     meeting" (0.878) to the cut. This is the scene's resting state. */
  .from('#s7pres', { opacity: 0, scale: 1.06, duration: 0.8, ease: 'power3.out' }, atf('s07_story', 0.841));
